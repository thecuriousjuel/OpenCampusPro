...
def get_dashboard_stats():
    ...
    paid_fees = (
        db.session.query(func.sum(Fee.amount)).filter_by(status="paid").scalar() or 0
    )
    ...
    if recent_attendance:
        ...
        attendance_rate = (
            (present_count / len(recent_attendance) * 100) if recent_attendance else 0
        )
    else:
        attendance_rate = 0
...
def get_attendance_report():
    ...
    current_app.logger.info(
        f"Generating attendance report - Start: {start_date}, End: {end_date}, Batch ID: {batch_id}"
    )
...
def get_fees_report():
    ...
    current_app.logger.info(
        f"Generating fees report - Start: {start_date}, End: {end_date}, Status: {status}"
    )
    ...
    return jsonify(
        {
            ...
            "summary": {
                ...
                "collection_rate": round(
                    (paid_amount / total_amount * 100) if total_amount > 0 else 0, 2
                ),
            },
        }
    ), 200
