...
def register():
    ...
    current_app.logger.info(
        f"Registration attempt for username: {data.get('username', 'N/A')}"
    )
    ...
    # Create new user
    user = User(
        username=data["username"], email=data["email"], role=data.get("role", "admin")
    )
    ...
    try:
        ...
        current_app.logger.error(
            f"Registration error for user '{data.get('username')}': {str(e)}",
            exc_info=True,
        )
