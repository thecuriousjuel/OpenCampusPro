...
def get_marks():
    ...
    current_app.logger.info(
        f"Fetching marks - Batch ID: {batch_id}, Student ID: {student_id}, Page: {page}"
    )
...
def create_mark():
    ...
    current_app.logger.info(
        f"Creating new mark: Student ID {data.get('student_id')}, Batch ID {data.get('batch_id')}"
    )

    if (
        not data
        or not data.get("student_id")
        or not data.get("batch_id")
        or data.get("marks_obtained") is None
    ):
    ...
    if not student:
        current_app.logger.warning(
            f"Mark creation failed: Student ID {data['student_id']} not found"
        )
    ...
    if not batch:
        current_app.logger.warning(
            f"Mark creation failed: Batch ID {data['batch_id']} not found"
        )
    ...
    if existing_mark:
        current_app.logger.warning(
            f"Mark creation failed: Mark already exists for Student ID {data['student_id']} in Batch ID {data['batch_id']}"
        )
        return jsonify(
            {"error": "Mark already exists for this student in this batch"}
        ), 400

    mark = Mark(
        student_id=data["student_id"], batch_id=data["batch_id"], marks_obtained=marks
    )
    ...
    try:
        ...
        db.session.commit()
        current_app.logger.info(
            f"Mark created successfully: Student ID {mark.student_id}, Batch ID {mark.batch_id}, Marks: {mark.marks_obtained} (ID: {mark.id})"
        )
        return jsonify(
            {"message": "Mark created successfully", "mark": mark.to_dict()}
        ), 201
...
def update_mark(id):
    ...
    try:
        db.session.commit()
        current_app.logger.info(
            f"Mark updated successfully: ID {mark.id}, New marks: {mark.marks_obtained}"
        )
        return jsonify(
            {"message": "Mark updated successfully", "mark": mark.to_dict()}
        ), 200
    except Exception as e:
        db.session.rollback()
        current_app.logger.error(
            f"Error updating mark ID {id}: {str(e)}", exc_info=True
        )
        return jsonify({"error": str(e)}), 500
...
def delete_mark(id):
    ...
    try:
        ...
        db.session.commit()
        current_app.logger.info(
            f"Mark deleted successfully: Student ID {student_id}, Batch ID {batch_id}, Marks: {marks_obtained} (ID: {id})"
        )
        return jsonify({"message": "Mark deleted successfully"}), 200
    except Exception as e:
        db.session.rollback()
        current_app.logger.error(
            f"Error deleting mark ID {id}: {str(e)}", exc_info=True
        )
        return jsonify({"error": str(e)}), 500
...
def get_analytics():
    ...
    if not marks:
        return jsonify(
            {"total_students": 0, "passed": 0, "failed": 0, "median_marks": 0}
        ), 200
