...
def get_courses():
    ...
    current_app.logger.info(
        f"Fetching courses - Page: {page}, Per page: {per_page}, Search: '{search}'"
    )
...
def create_course():
    ...
    current_app.logger.info(
        f"Creating new course: {data.get('name', 'N/A')} ({data.get('code', 'N/A')})"
    )
    ...
    if Course.query.filter_by(code=data["code"]).first():
        current_app.logger.warning(
            f"Course creation failed: Code '{data['code']}' already exists"
        )
    ...
    try:
        ...
        db.session.commit()
        current_app.logger.info(
            f"Course created successfully: {course.name} (ID: {course.id})"
        )
        return jsonify(
        ...
        current_app.logger.error(
            f"Error creating course '{data.get('name')}': {str(e)}", exc_info=True
        )
...
def update_course(id):
    ...
    try:
        db.session.commit()
        current_app.logger.info(
            f"Course updated successfully: {course.name} (ID: {course.id})"
        )
        return jsonify(
        ...
        db.session.rollback()
        current_app.logger.error(
            f"Error updating course ID {id}: {str(e)}", exc_info=True
        )
        return jsonify({"error": str(e)}), 500
...
def delete_course(id):
    ...
    try:
        ...
        db.session.commit()
        current_app.logger.info(
            f"Course deleted successfully: {course_name} (ID: {id})"
        )
        return jsonify({"message": "Course deleted successfully"}), 200
    except Exception as e:
        db.session.rollback()
        current_app.logger.error(
            f"Error deleting course ID {id}: {str(e)}", exc_info=True
        )
        return jsonify({"error": str(e)}), 500
