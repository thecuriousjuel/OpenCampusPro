...
def get_students():
    ...
    current_app.logger.info(f"Fetching students - Page: {page}, Per page: {per_page}, Search: '{search}'")
...
def create_student():
    ...
    if Student.query.filter_by(email=data['email']).first():
        current_app.logger.warning(f"Student creation failed: Email '{data['email']}' already exists in students")
    ...
    try:
        ...
        db.session.commit()
        current_app.logger.info(f"Student created successfully: {student.name} (ID: {student.id})")
        return jsonify({
        ...
        db.session.rollback()
        current_app.logger.error(f"Error creating student '{data.get('name')}': {str(e)}", exc_info=True)
...
def update_student(id):
    ...
    try:
        db.session.commit()
        current_app.logger.info(f"Student updated successfully: {student.name} (ID: {student.id})")
        return jsonify({
        ...
        db.session.rollback()
        current_app.logger.error(f"Error updating student ID {id}: {str(e)}", exc_info=True)
        return jsonify({'error': str(e)}), 500
...
def delete_student(id):
    ...
    try:
        ...
        db.session.commit()
        current_app.logger.info(f"Student deleted successfully: {student_name} (ID: {id})")
        return jsonify({'message': 'Student deleted successfully'}), 200
    except Exception as e:
        db.session.rollback()
        current_app.logger.error(f"Error deleting student ID {id}: {str(e)}", exc_info=True)
        return jsonify({'error': str(e)}), 500
