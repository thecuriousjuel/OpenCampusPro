...
def get_fees():
    ...
    current_app.logger.info(
        f"Fetching fees - Student ID: {student_id}, Status: {status}, Page: {page}"
    )
    ...
    # Update overdue fees
    today = date.today()
    overdue_fees = Fee.query.filter(Fee.status == "pending", Fee.due_date < today).all()
...
def create_fee():
    ...
    current_app.logger.info(
        f"Creating new fee record for student ID: {data.get('student_id', 'N/A')}"
    )
    ...
    if not student:
        current_app.logger.warning(
            f"Fee creation failed: Student ID {data['student_id']} not found"
        )
    ...
    try:
        ...
        db.session.commit()
        current_app.logger.info(
            f"Fee record created successfully: Student ID {fee.student_id}, Amount: {fee.amount} (ID: {fee.id})"
        )
        return jsonify(
            {"message": "Fee record created successfully", "fee": fee.to_dict()}
        ), 201
...
def pay_fee(id):
    ...
    try:
        db.session.commit()
        current_app.logger.info(
            f"Payment recorded successfully for fee ID {id}: Student ID {fee.student_id}, Amount: {fee.amount}"
        )
        return jsonify(
            {"message": "Payment recorded successfully", "fee": fee.to_dict()}
        ), 200
    except Exception as e:
        db.session.rollback()
        current_app.logger.error(
            f"Error recording payment for fee ID {id}: {str(e)}", exc_info=True
        )
        return jsonify({"error": str(e)}), 500
...
def delete_fee(id):
    ...
    try:
        ...
        db.session.commit()
        current_app.logger.info(
            f"Fee record deleted successfully: Student ID {student_id}, Amount: {fee_amount} (ID: {id})"
        )
        return jsonify({"message": "Fee record deleted successfully"}), 200
    except Exception as e:
        db.session.rollback()
        current_app.logger.error(
            f"Error deleting fee record ID {id}: {str(e)}", exc_info=True
        )
        return jsonify({"error": str(e)}), 500
